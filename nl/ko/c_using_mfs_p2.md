...
{{site.data.keyword.mobilefoundation_short}}: Professional 1 Application 서비스 인스턴스가 작성되고 나면 *개요* 페이지가 표시됩니다. 여기서 {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스가 연결해야 하는 {{site.data.keyword.Db2_on_Cloud_short}}(**Lite** 플랜 이외의 모든 플랜) 또는 {{site.data.keyword.composeForPostgreSQL}} 서비스 인스턴스에 대한 연결 정보를 지정해야 합니다.
