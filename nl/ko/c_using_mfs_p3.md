...
Developer Pro 플랜을 사용하여 {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스를 작성한 후에 {{site.data.keyword.Bluemix_notm}}의 `Overview` 페이지에 액세스하십시오. 이 페이지에서는 서비스를 시작하는 데 유용한 튜토리얼과 동영상을 제공합니다.

## 전제 조건
{: #prerequisites_p3}

{{site.data.keyword.mobilefoundation_short}}: Developer Pro 서비스 인스턴스를 구성하기 전에 다음을 고려하십시오.
* {{site.data.keyword.mobilefoundation_short}}: Developer Pro는 {{site.data.keyword.Db2_on_Cloud_short}} {{site.data.keyword.Bluemix_notm}} 플랜과 함께 사용하는 경우에만 지원됩니다.
...
Developer Pro 플랜을 사용하여 {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스를 작성한 후에 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스에 대한 연결 정보를 지정해야 하는 *Overview* 페이지가 표시됩니다. {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스가 지정된 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스에 연결되어야 합니다.

기존 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스가 없는 경우 새로 작성할 수 있습니다.

다음 단계에 따라 Db2 on Cloud 서비스 인스턴스를 작성하십시오.

1. *Overview* 페이지에서 **새 서비스 작성** 섹션을 선택하십시오.

+ 고가용성 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스가 필요한 경우 **고가용성 구성** 옵션에서 `Yes`를 선택하십시오.
...
다음 단계를 수행하여 기존 또는 새 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스에 연결하십시오.
...
{: #adding_analytics_server_p3}
 ...
 이제 {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스에 Mobile Analytics 서비스 인스턴스를 연결하여 {{site.data.keyword.mobilefirst}} 서버에서 모바일 애플리케이션을 모니터링할 수 있습니다.

 Developer Pro 플랜이 Mobile Analytics 서비스 인스턴스를 작성합니다.
...
* **분석 추가**를 클릭하여 {{site.data.keyword.mobilefoundation_short}} 서비스 인스턴스에 Mobile Analytics 서비스 인스턴스를 추가하십시오.
...
Mobile Analytics 서비스에 대한 자세한 정보는 [MobileFirst Foundation Operational Analytics ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://mobilefirstplatform.ibmcloud.com/tutorials/en/foundation/8.0/analytics/){: new_window}를 참조하십시오.
...
> **참고**: 앱과 어댑터에 대한 정보를 포함된 이전 서버 인스턴스의 데이터가 구성된 {{site.data.keyword.Db2_on_Cloud_short}} 서비스 인스턴스에서 지속됩니다. 이 데이터는 서버를 다시 작성하는 데 사용됩니다.
...
*	**토폴로지** 탭에서 필요한 서버 크기와 메모리를 선택할 수 있습니다. 기본 서버는 1GB의 메모리로 작성됩니다.
  ...
  - **인스턴스**는 작성된 노드의 수를 표시합니다. 이 필드는 {{site.data.keyword.mobilefoundation_short}}: Developer Pro에서 편집할 수 없습니다. Developer Pro 플랜에서 노드 수는 기본적으로 **1**입니다.
