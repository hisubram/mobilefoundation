...
O Mobile Analytics é um recurso integrado dentro do serviço {{ site.data.keyword.mobilefoundation_short }}. O Mobile Analytics fornece insights importantes sobre o uso e o desempenho de aplicativos para desenvolvedores de aplicativos móveis e proprietários de aplicativos.

Será necessário instrumentar seu aplicativo móvel para começar a usar o recurso Mobile Analytics para monitorar o uso do aplicativo, o desempenho e para obter outras estatísticas. A instrumentação de seu aplicativo tem as etapas a seguir: 
...
As seções a seguir fornecerão os detalhes para essas etapas, para cada uma das plataformas suportadas.
...
[![Central do Maven](https://maven-badges.herokuapp.com/maven-central/com.ibm.mobile.foundation/ibmmobilefirstplatformfoundationanalytics/badge.svg)](https://maven-badges.herokuapp.com/maven-central/com.ibm.mobile.foundation/ibmmobilefirstplatformfoundationanalytics)
{: android}
...
{: android}
   ...
   Em seguida, use o SDK cliente do Mobile Analytics para configurar ou inicializar a captura de dados de analítica.  
...
2. Para capturar eventos de ciclo de vida do aplicativo, como sessão de aplicativo e informações de travamento, configure seu aplicativo incluindo o seguinte:
  ```Java
...
3. Para capturar eventos de rede que capturam as interações de rede de aplicativos, configure seu aplicativo incluindo o seguinte: 
  ```Java
...
4. Agora você inicializou seu aplicativo para capturar os dados de analítica.  Em seguida, é necessário enviar os dados capturados para o serviço Mobile Analytics.
  ...
  Você é livre para decidir quando chamar essa API em seu fluxo de aplicativo para enviar os dados de analítica capturados para o serviço Mobile Analytics.  Até que isso envie toda a análise de dados capturada, os dados serão armazenados localmente no dispositivo.
  {: android}

5. Para capturar e enviar logs do aplicativo ao serviço Mobile Analytics, use as APIs do criador de logs do SDK cliente do Mobile Analytics.     
...
6.  Para obter insights sobre padrões de integração de usuário (novos usuários versus usuários retornados), deve-se associar uma identidade do usuário à sua sessão de aplicativo.  Isso pode ser feito chamando a API a seguir
    ```java
    ...
    Observe que todos os dados de analítica capturados e armazenados localmente são enviados para o serviço Mobile Analytics somente quando a API WLAnalytics.send() é chamada.
    {: android}

7.  Para obter insights sobre os padrões de interação de rede HTTP de seus aplicativos, como APIs HTTP chamadas, número de solicitações e tempos médios de resposta, deve-se usar a classe WLResourceRequest do SDK cliente do Mobile Foundation Service para fazer as chamadas HTTP.  Embora você possa ter inicializado o Analytics para a captura de eventos de Rede, usar WLResourceRequest permite que o Mobile Analytics conecte as chamadas de rede e capture os dados relevantes.  Aqui está como se deve fazer suas chamadas HTTP.
    ```java
...
8.  Para obter o Crash Analytics e os logs, seria possível chamar a API a seguir durante o início do seu aplicativo para verificar se a sessão anterior tinha travado e assim enviar os logs de travamento de captura para o serviço Mobile Analytics.
    ```java
...
9.  Para definir a Analítica customizada e definir seus próprios dados de analítica para além do que é suportado inerentemente no SDK cliente, seria possível usar a API de criação de log customizada
    ```java
...
10. Use o Feedback do usuário no app para aprofundar a análise de desempenho do seu aplicativo.   É possível ativar os **usuários e testadores** do aplicativo para fornecer feedback contextual rico aos proprietários de app. **Proprietários de app** obtêm feedback em tempo real de seus usuários sobre a experiência de uso do aplicativo na qual os **Proprietários de app** e **Desenvolvedores** podem atuar posteriormente.  Isso traz uma agilidade significativa para a manutenção do aplicativo.  Use a API a seguir para alternar seu aplicativo para o Modo de feedback interativo em qualquer manipulador de ações em seu aplicativo, por exemplo, ao manipular o clique de um botão ou a seleção de um item de menu.
    ```java
...
1. Inclua o seguinte no arquivo pod existente, localizado na raiz do projeto Xcode
   ```bash
   use_frameworks!
   platform :ios, 8.0
...
5. Para capturar e enviar logs do aplicativo ao serviço Mobile Analytics, use as APIs do criador de logs do SDK cliente do Mobile Analytics.
   ...
   {: ios}
    ...
    Siga [o tutorial ![Ícone de link externo](../../icons/launch-glyph.svg "Ícone de link externo")](https://mobilefirstplatform.ibmcloud.com/tutorials/it/foundation/8.0/application-development/client-side-log-collection/ios/) para obter fragmentos de código para o Criador de logs para incluir recursos de criação de log em aplicativos iOS.
...
6.  Para obter insights sobre padrões de integração de usuário (novos usuários versus usuários retornados), deve-se associar uma identidade do usuário à sua sessão de aplicativo.  Isso pode ser feito chamando a API a seguir
    ...
    Observe que todos os dados de analítica capturados e armazenados localmente são enviados para o serviço Mobile Analytics somente quando a API WLAnalytics.sharedInstance().send() é chamada.
    {: ios}
...
2. Para ativar a captura dos eventos de ciclo de vida, ele deve ser inicializado na plataforma nativa do app Cordova.
    * Para a plataforma Android:
      ...
      * Procure o método `onCreate` e siga as etapas abaixo para ativar as atividades `LIFECYCLE` e `NETWORK`:
        * Para ativar o log de eventos do ciclo de vida do cliente:
          ```Java
          WLAnalytics.addDeviceEventListener(DeviceEvent.LIFECYCLE);
          ```
          {: codeblock}
          {: cordova}
        * Para ativar a criação de log de eventos de rede do cliente:
          ```Java
    ...
    * Para a plataforma do iOS:
      ...
      * Siga as etapas abaixo para ativar as atividades `LIFECYCLE` e `NETWORK`.
        * Para ativar o log de eventos do ciclo de vida do cliente:
          ```Swift
          WLAnalytics.sharedInstance().addDeviceEventListener(LIFECYCLE);
          ```
...
4. Para capturar e enviar logs do aplicativo ao serviço Mobile Analytics, use as APIs do criador de logs do SDK cliente do Mobile Analytics.
   A estrutura de criação de log do SDK cliente do Mobile Analytics suporta os níveis de log a seguir, que estão listados do menos para o mais detalhado, com as diretrizes de uso recomendadas:
    * FATAL - Use para travamentos ou interrupções irrecuperáveis. O nível FATAL é reservado para erros irrecuperáveis de criação de log, que aparecem para usuários como um travamento do aplicativo
    * ERROR - Use para exceções inesperadas ou erros de protocolo de rede inesperados
    * WARN - Use para registrar os avisos de uso que não são considerados erros críticos, como uso de APIs descontinuadas ou resposta de rede lenta
...
5.  Para obter insights sobre padrões de integração de usuário (novos usuários versus usuários retornados), deve-se associar uma identidade do usuário à sua sessão de aplicativo.  Não há nenhuma API específica disponível no nível de Javascript. Mas isso pode ser feito no código nativo chamando a API a seguir:
    ...
    **iOS:**
    ...
    Observe que todos os dados de analítica capturados e armazenados localmente são enviados para o serviço Mobile Analytics somente quando a API WL.Analytics.send() no nível de javascript [ou] a API WLAnalytics.send() no android nativo [ou] a API WLAnalytics.sharedInstance().send() no iOS nativo é chamada.
    {: cordova}
...
7. Para definir a Analítica customizada e definir seus próprios dados de analítica para além do que é suportado inerentemente no SDK cliente, seria possível usar a API de criação de log customizada
    ```Javascript
...
4. Agora você inicializou seu aplicativo para capturar os dados de analítica.  Em seguida, é necessário enviar os dados capturados para o serviço Mobile Analytics. Use a API a seguir para enviar dados de analítica para o serviço Mobile Analytics.
...
6. Para obter insights sobre padrões de integração de usuário (novos usuários versus usuários retornados), deve-se associar uma identidade do usuário à sua sessão de aplicativo.  Isso pode ser feito chamando a API a seguir
    ...
    Observe que todos os dados de analítica capturados e armazenados localmente são enviados para o serviço Mobile Analytics somente quando a API ibmmfpfanalytics.send() é chamada.
    {: web}
...
8.  Para definir a Analítica customizada e definir seus próprios dados de analítica além do que é suportado inerentemente no SDK do Cliente, seria possível usar a API de criação de log customizada:
