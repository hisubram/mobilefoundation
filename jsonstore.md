...
![JSONStore feature workflow](images/jstore_workflow.jpg)
...
An example use case would be various employees that share a physical device (for example an iPad or Android tablet) and MobileFirst application. If the employees work different shifts and handle private data from different customers while they use the MobileFirst application, multiple user support is useful.
...
### Objective-C
{: #objective-c }
When you use the Native iOS API for JSONStore, all operations are added to a synchronous dispatch queue. This behavior ensures that operations that touch the store are executed in order on a thread that is not the main thread. For more information, see the Apple documentation at [Grand Central Dispatch (GCD) ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://developer.apple.com/library/ios/documentation/Performance/Reference/GCD_libdispatch_Ref/Reference/reference.html#//apple_ref/c/func/dispatch_sync){: new_window}.
...
## Troubleshooting
{: #troubleshooting }
For more information, see the [JSONStore troubleshooting ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://mobilefirstplatform.ibmcloud.com/tutorials/en/foundation/8.0/troubleshooting/jsonstore){: new_window} section.
